...
class _SimpleSegmentationModel(nn.Module):
    ...
    def forward(self, x, istrain=False):
        ...
        if 'low_level' in features:  # Replace 'layer_1' with the appropriate layer name
            stage1 = features['low_level']
            B_out = self.boundary(stage1)
            features['low_level'] = stage1 + B_out.repeat_interleave(int(stage1.shape[1]), dim=1)
        ...
        for name, module in self.named_children():
            ...
            if name in self.return_layers:
                ...
                if name == 'stage4' and self.hrnet_flag: # In HRNetV2, we upsample and concat all outputs streams together
                    ...
                    # if name == 'layer1':  # or the layer you choose to apply BEM on
                    #     print(name)
                    #     stage1 = out[out_name]
                    #     B_out = self.boundary(stage1)
                    #     out[out_name] = stage1 + B_out.repeat_interleave(int(stage1.shape[1]), dim=1)
                ...
                # if self.boundary_module is not None:
                # if 'stage1' in out:  # Example, modify this based on your model
                #     stage1_output = out['stage1']
                #     boundary_output = self.boundary_module(stage1_output)
                #     out['boundary'] = boundary_output
